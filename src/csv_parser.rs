...
/// This does not contain any data, but does have an open file handle. This is
/// for handling large (or potentially large) csv file in a streaming manner.
...
impl CsvDataBlock {
    ...
    /// Return the length of the vectors (they all share the same length by
    /// construction). Return zero if there are no vectors.
